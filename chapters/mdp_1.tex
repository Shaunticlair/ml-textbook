...
\setcounter{section}{0}
    ...
    In the RNN chapter, we introduced \vocab{state machines (SMs)}: a system for keeping track of our \textbf{current situation}, using a "state". 

    \begin{itemize}
        \item This state also contains information about the \purp{past}: our \textbf{present} state is influenced by past data.
    \end{itemize}

    In particular, we used \vocab{finite state machines}. \\
    ...
    \subsection{Sequence-to-sequence perspective}
        ...
        At the time, this was used in RNNs, to process \gren{sequences} of information:
        ...
        \begin{itemize}
            ...
            \item While our output is \purp{based on} that new information.
        \end{itemize}
...
\section{Definition and Value Functions}
    ...
    \subsection{Finite Horizon, $H=2$}
        ...
        \begin{concept}
            Our \gren{state transition} is \gren{non-deterministic}.

            \begin{itemize}
                \item That means that we could end up in \gren{several new states} $\blu{s'}$. 
            \end{itemize}
